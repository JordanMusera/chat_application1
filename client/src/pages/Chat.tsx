...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    if (chatId !== 0) {
      ...
      console.log("Joined existing chat:", chatId);
    } else {
      const newId = uuidv4();
      setNewConvId(newId);
    ...
    const handleReceiveMessage = (data: Message) => {
      setSelectedChat((prev: any) => {
        ...
        if (chatId === 0) {
          ...
          setSearchWord("");
          console.log("New chat created and joined:", data.chat_id);
          return newChat;
        }
  ...
  const search = async () => {
    try {
      const res = await fetch(
        "http://192.168.0.159:5000/users/getSearchUsers",
        {
          method: "POST",
          headers: {
  ...
  const getUser = async () => {
    const res = await fetch("http://192.168.0.159:5000/users/getUser", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
    ...
    if (chatId === 0) {
      setSelectedChat((prev) =>
        prev
          ? {
              ...prev,
              messages: [...prev.messages, messageData as any],
            }
          : prev
      );
    }
